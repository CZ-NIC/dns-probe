find_package(PkgConfig REQUIRED)

set(OLD_PKGCFG_ENV $ENV{PKG_CONFIG_PATH})
unset(ENV{PKG_CONFIG_PATH})

foreach(PATH IN LISTS CMAKE_SYSTEM_PREFIX_PATH CMAKE_PREFIX_PATH)
    set(ENV{PKG_CONFIG_PATH} "${PATH}/lib/pkgconfig:${PATH}/lib64/pkgconfig:$ENV{PKG_CONFIG_PATH}")
endforeach()

set(ENV{PKG_CONFIG_PATH} "$ENV{PKG_CONFIG_PATH}:${OLD_PKGCFG_ENV}")
pkg_search_module(PARQUET parquet)
set(ENV{PKG_CONFIG_PATH} "${OLD_PKGCFG_PATH}")

find_package_handle_standard_args(Parquet DEFAULT_MSG PARQUET_FOUND)

if (PARQUET_FOUND)
    add_library(Parquet::Parquet INTERFACE IMPORTED)
    set_property(TARGET Parquet::Parquet PROPERTY INTERFACE_INCLUDE_DIRECTORIES ${PARQUET_INCLUDE_DIRS})
    set_property(TARGET Parquet::Parquet PROPERTY INTERFACE_LINK_LIBRARIES ${PARQUET_LIBRARIES})
    set_property(TARGET Parquet::Parquet PROPERTY INTERFACE_COMPILE_DEFINITIONS ${PARQUET_DEFINITIONS})
    set_property(TARGET Parquet::Parquet PROPERTY INTERFACE_COMPILE_OPTIONS ${PARQUET_CFLAGS})
endif()
